...
public class PongLogic : MonoBehaviour
{
	...
	void Update ()
	{
		...
		for (int i = ballsIntern.Count - 1; i >= 0; i--) {
			...
			if (ball.renderer.bounds.min.x <= left.renderer.bounds.min.x) { // eigtl bound max, aber buggt

				ballsIntern.Remove (ball);
				ballSpin.RemoveAt(i);
				Destroy (ball);
                Game.getInstance().player2GameScore++; 
			} else if (ball.renderer.bounds.max.x >= right.renderer.bounds.max.x) { // eigtl bound min, aber buggt
